...
def get_yarn_logs(applicationId, appOwner):
    logs = subprocess.check_output(['yarn', 'logs', '-applicationId', '{0}'.format(applicationId), '-appOwner', '{0}'.format(appOwner)])
...
def upload_logs(applicationId, channel="#test", token='50-char-valid-token'):
    curl_command = "curl -F file=@{0}.log -F channels={1} -F token={2} https://slack.com/api/files.upload".format(applicationId, channel, token).split()
    try:
...
def generate_attachment(app):
    ...
    return [{ "fallback": "Application {0}. Name: {1}. ".format(app['id'], app['name']),
            ...
            "fields": [
                    ...
                    {
                        "title": "Start Time",
                        "value": "{0}".format(datetime.fromtimestamp(int(app['startedTime'])/1000).strftime('%Y-%m-%d %H:%M:%S %Z')),
                        "short": True
                    },
                    {
                        "title": "End Time",
                        "value": "{0}".format(datetime.fromtimestamp(int(app['finishedTime'])/1000).strftime('%Y-%m-%d %H:%M:%S %Z')),
                        "short": True
                    }
                ],
            "color": color
        ...
        for app in cleaned_apps.keys():
            if cleaned_apps[app]['name'].startswith(yarn_app_name_filter):
                try:
                    ...
                    if cleaned_apps[app]['state'] == "FINISHED" and cleaned_apps[app]['finalStatus'] == "FAILED" and (prev_apps[app]['state'] == "RUNNING" or app not in prev_apps):
                    ...
                    if app not in prev_apps and cleaned_apps[app]['state'] == "FINISHED" and cleaned_apps[app]['finalStatus'] == "FAILED" :
                        print cleaned_apps[app]
                        print "generating attachment"
                        attachment = generate_attachment(cleaned_apps[app])
                        print "posting to slack"
